...
int main(int argc, char *argv[])
{
    ...
    if (argc < 2)
    {
        if (worldRank == 0)
        {
            cout << "Usage: mpirun -np <processes> ./mpi_matrix N [write_output]\n";
        }
        MPI_Finalize();
        return 1;
    }
    ...
    // Each process computes its assigned rows of matrix C
    ...
    // Gather result rows back to the root process
    ...
    // Root reports the slowest participating process time
    ...
    if (worldRank == 0)
    {
        ...
        if (writeOutput == 1)
        {
            writeMatrixToFile(matrixC, matrixSize, "C_mpi.txt");
        }

        cout << "N = " << matrixSize << '\n';
        cout << "Processes = " << worldSize << '\n';
        cout << "Threads per process = 1\n";
        cout << "Method = MPI\n";
        cout << "Execution time = " << (long long)(maxElapsed * 1000000.0)
             << " us (" << maxElapsed * 1000.0 << " ms)\n";
        cout << "Checksum = " << checksum << '\n';
        cout << "Output = " << (writeOutput == 1 ? "C_mpi.txt" : "not written") << '\n';
    }
